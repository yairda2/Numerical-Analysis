...
table12 = ((1, 0.8415), (2, 0.9093),
           (3, 0.1411), (4, -0.7568),
           (5, -0.9589), (6, -0.2794))
table3 = ((2.25, 0.112463), (2.3, 0.167996), (2.7, 0.222709))
...
def Interrogation_of_Lagrange(X):
	x = 2.4  # you can change your param to find here
	...
	print(f'Interrogation_of_Lagrange:\n'
	      f'The x is: {x}\n'
	      f'L0 = ((x - X[1][0]) / (X[0][0] - X[1][0])) * ((x - X[2][0]) / (X[0][0] - X[2][0])) = {L0}\n'
	      f'L1 = ((x - X[0][0]) / (X[1][0] - X[0][0])) * ((x - X[2][0]) / (X[1][0] - X[2][0])) = {L1}\n'
	      f'L2 = ((x - X[0][0]) / (X[2][0] - X[0][0])) * ((x - X[1][0]) / (X[2][0] - X[1][0])) = {L2}\n')
...
def Spline_Kobe(arr, x_f1):
    ...
    for r in range(len(multiply_elementary_matrix)):
        for c in range(len(multiply_elementary_matrix)):
            ...
            print(f'the final result each time = {final_result1[r]}')
...
def menu():
	...
	print(f'\nMethod: "Interrogation of Lagrange" is result is -> L0+L1+L2 = {Interrogation_of_Lagrange(table3)}\n***********************************\n')
	#print(f'\nMethod: "Neville_Algorithm" is -> {Neville_Algorithm(table4)}\n***********************************\n')
	print(f'Method: "Spline_Kobe method"')
	print(f'The x we look for:{x_f}\nthe args are:{values_table}\n'
	      f'The result is: {Spline_Kobe(values_table, x_f)}')
